...
"""
grouped_data = filtered_df.groupby(filtered_df.iloc[:, Loan_Type]).iloc[:, Loan_Amount].sum().reset_index()
plt.figure(figsize=(10, 6))  
plt.bar(df['Loan_Type'], df['Loan_Amount'])  
plt.xlabel('Loan_Type')
plt.ylabel('Loan_Amount')
plt.title('Loan_Amount vs Loan_Type')
